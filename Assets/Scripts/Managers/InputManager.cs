...
public class InputManager : MonoBehaviour
{
	...
	public void TrySetNewKeybind(string buttonName, int buttonNum, KeyCode key)
	{
		...
		if (key == KeyCode.Escape)
		{
			...
			GameManager.Instance.playerNotifsManager.DisplayNotificationMessage("key Rebinding Canceled", 1f);
		}
		...
		{
			MenuUIManager.Instance.CancelKeybindButtonDisplay(buttonNum, InputManager.Instance.keyBindDictionary[Instance.keybindNames[buttonNum]]);
			GameManager.Instance.playerNotifsManager.DisplayNotificationMessage("key already bound", 1f);
		}
		keyToRebind = "";
		buttonNumToRebind = -1;
	}
